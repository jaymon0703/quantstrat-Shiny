...
ui <- fluidPage(
  mainPanel(
    textInput("caption", "Caption", "AAPL"),
...
server <- function(input, output) {
  ...
  output$ggplot <- renderPlot({
    ...
    stock.str <- as.character(input$caption)
    ...
    AAPL <- getSymbols(stock.str,from=startDate, auto.assign = FALSE)
    # saveSymbols(Symbols=symbols, file.path=stop(getwd()))
    for(i in stock.str)
      assign(i, adjustOHLC(get(i),use.Adjusted=TRUE))
    
    start_t<-Sys.time()
    assign("symbols", symbols, .GlobalEnv)
